 ...
 ****************************************************************************************
 */
extern void fee0_fee1_0x2a99_cb(uint8_t* buf);
extern void fee0_fee2_0x2a85_cb(uint8_t* buf);
extern void fee0_fee3_0x2a9f_cb(uint8_t* buf);
extern void fee0_fee4_0x2a8c_cb(uint8_t* buf);
extern void fee0_fee5_0x2a8e_cb(uint8_t* buf);
...
static int fee1_writer_req_handler(ke_msg_id_t const msgid,
                                     struct fee0s_fee1_writer_ind *param,
                                     ke_task_id_t const dest_id,
                                     ke_task_id_t const src_id)
{
    // Drop the message
  UART_PRINTF("FEE1 param->value = 0x ");
  
  for(uint8_t i = 0;i < param->length;i++)
  {
    UART_PRINTF("%02x ",param->fee1_value[i]);
  }
  UART_PRINTF("\r\n");

  fee0_fee1_0x2a99_cb(param->fee1_value);
                                     ...
                                     ke_task_id_t const src_id)
{
	...
	UART_PRINTF("\r\n");
		fee0_fee2_0x2a85_cb(param->fee2_value);
    return (KE_MSG_CONSUMED);
}
static int fee3_writer_req_handler(ke_msg_id_t const msgid,
                                     struct fee0s_fee3_writer_ind *param,
                                     ke_task_id_t const dest_id,
                                     ke_task_id_t const src_id)
{
    // Drop the message
  UART_PRINTF("FEE3 param->value = 0x ");
  
  for(uint8_t i = 0;i < param->length;i++)
  {
    UART_PRINTF("%02x ",param->fee3_value[i]);
  }
  UART_PRINTF("\r\n");
    fee0_fee3_0x2a9f_cb(param->fee3_value);
    return (KE_MSG_CONSUMED);
}

static int fee4_writer_req_handler(ke_msg_id_t const msgid,
                                     struct fee0s_fee4_writer_ind *param,
                                     ke_task_id_t const dest_id,
                                     ke_task_id_t const src_id)
{
    // Drop the message
  UART_PRINTF("FEE4 param->value = 0x ");
  
  for(uint8_t i = 0;i < param->length;i++)
  {
    UART_PRINTF("%02x ",param->fee4_value[i]);
  }
  UART_PRINTF("\r\n");
    fee0_fee4_0x2a8c_cb(param->fee4_value);
    return (KE_MSG_CONSUMED);
}

static int fee5_writer_req_handler(ke_msg_id_t const msgid,
                                     struct fee0s_fee5_writer_ind *param,
                                     ke_task_id_t const dest_id,
                                     ke_task_id_t const src_id)
{
    // Drop the message
  UART_PRINTF("FEE5 param->value = 0x ");
  
  for(uint8_t i = 0;i < param->length;i++)
  {
    UART_PRINTF("%02x ",param->fee5_value[i]);
  }
  UART_PRINTF("\r\n");
    fee0_fee5_0x2a8e_cb(param->fee5_value);
    return (KE_MSG_CONSUMED);
}
...
const struct ke_msg_handler app_fee0_msg_handler_list[] =
{
    ...
    {FEE0S_FEE1_WRITER_REQ_IND,    (ke_msg_func_t)fee1_writer_req_handler},
    ...
    {FEE0S_FEE4_WRITER_REQ_IND,   (ke_msg_func_t)fee4_writer_req_handler},
    {FEE0S_FEE5_WRITER_REQ_IND,   (ke_msg_func_t)fee5_writer_req_handler},
