 ...
 * @file   ff60s_task.c
 *
 * @brief FF60 Server Role Task Implementation.
 *
...
static int ff60s_ff61_level_upd_req_handler(ke_msg_id_t const msgid,
                                            struct ff60s_ff61_level_upd_req const *param,
                                            ...
                                            ke_task_id_t const src_id)
{
    ...
    if(state == FF60S_IDLE)
    {
        ...
        ke_state_set(dest_id, FF60S_BUSY);						
		ff60s_notify_ff61_lvl(ff60s_env, param);
		ke_state_set(dest_id, FF60S_IDLE);   
                                      ...
                                      ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    if (status == GAP_ERR_NO_ERROR)
    {
                    ...
                    || (ntf_cfg == PRF_CLI_START_IND)))
        {
            ...
            struct ff60s_ff61_level_ntf_cfg_ind * ind = KE_MSG_ALLOC(FF60S_FF61_LEVEL_NTF_CFG_IND,
                    prf_dst_task_get(&(ff60s_env->prf_env), conidx), dest_id,
                    ff60s_ff61_level_ntf_cfg_ind);
            ind->conidx = conidx;
            ind->ntf_cfg = ff60s_env->ntf_cfg[conidx];
		...
		else if (att_idx == FF60S_IDX_FF62_LVL_VAL)
		{
			...
			struct ff60s_ff62_writer_ind *ind = KE_MSG_ALLOC(FF60S_FF62_WRITER_REQ_IND,
			        prf_dst_task_get(&(ff60s_env->prf_env), conidx),
			        dest_id, ff60s_ff62_writer_ind);
			
			// Fill in the parameter structure	
			memcpy(ind->ff62_value,&param->value[0],param->length);
			ind->conidx = conidx;
                                      ...
                                      ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    if (status == GAP_ERR_NO_ERROR)
    {
        ...
        {
            uint16_t ntf_cfg = (ff60s_env->ntf_cfg[conidx] & FF60_FF61_LVL_NTF_SUP) ? PRF_CLI_START_NTF : PRF_CLI_STOP_NTFIND;
                                 ...
                                 ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    {	
	    ...
	    struct ff60s_ff61_level_upd_rsp *rsp = KE_MSG_ALLOC(FF60S_FF61_LEVEL_UPD_RSP,
                                                         prf_dst_task_get(&(ff60s_env->prf_env), conidx),
                                                         dest_id, ff60s_ff61_level_upd_rsp);
    ...
    ke_state_set(dest_id, ke_state_get(dest_id) & ~FF60S_BUSY);
	
    return (KE_MSG_CONSUMED);
}

/// Default State handlers definition
const struct ke_msg_handler ff60s_default_state[] =
{
    {FF60S_FF61_LEVEL_UPD_REQ,      (ke_msg_func_t) ff60s_ff61_level_upd_req_handler},
...
const struct ke_state_handler ff60s_default_handler = KE_STATE_HANDLER(ff60s_default_state);
