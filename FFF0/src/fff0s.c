...
const struct attm_desc fff0_att_db[FFF0S_IDX_NB] =
{
  ...
  // fff1 Level Characteristic Declaration
    ...
    //  Characteristic Value
    [FFF0S_IDX_FFF4_LVL_VAL]   =   {ATT_USER_SERVER_CHAR_FFF4,PERM(RD, ENABLE)|PERM(WRITE_REQ, ENABLE), PERM(RI, ENABLE), FFF0_FFF2_DATA_LEN *sizeof(uint8_t)},
...
uint8_t fff0s_get_att_idx(uint16_t handle, uint8_t *att_idx)
{
	...
	else if(handle <= (hdl_cursor2 + FFF0S_IDX_FFF3_LVL_VAL))
	{
